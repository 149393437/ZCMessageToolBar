...
-(void)makeUI{
    ...
    leftTextView.mmFont=[UIFont systemFontOfSize:10];
    ...
    [leftVoiceImageView addSubview:leftControl];
    ...
    //右边
    rightImageView=[ZCControl createImageViewWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-40, 8, 30, 30) ImageName:nil];
    ...
    rightTextView.mmFont=[UIFont systemFontOfSize:10];
    ...
    [rightVoiceImageView addSubview:rightControl];
    ...
    timeLabel=[ZCControl createLabelWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 10) Font:10 Text:nil];
...
-(void)locationClick:(UIButton*)button{
    ...
    LocationViewController*vc=[[LocationViewController alloc]initWithLocation:CLLocationCoordinate2DMake([[array firstObject]floatValue], [[array lastObject]floatValue])];
    //移动到顶层VC上
...
-(UIImage*)imageNameToImageLeftOrRight:(BOOL)isLeft{
    ...
    NSString*bubbleImageName=[user objectForKey:@"bubble"];
    
    if ([user objectForKey:@"theme"]) {
        //如果连主题都没有,则采用默认的
    ...
    //读取路径theme
        if (bubbleImageName) {
            self.path=[NSString stringWithFormat:@"%@%@/%@",[NSString stringWithFormat:@"%@/",[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject]],bubbleImageName,@"chat_bubble_thumbnail.png"];
             UIImage*image=[UIImage imageWithContentsOfFile:self.path];
            //进行拉伸
            image=[image stretchableImageWithLeftCapWidth:40 topCapHeight:20];
            if (isLeft) {
                //单独气泡只有右边,需要进行翻转
            ...
            if (isLeft) {
                if ([user objectForKey:@"theme"]==nil) {
                 ...
                 self.path=[NSString stringWithFormat:@"%@%@/chat_recive_nor.png",[NSString stringWithFormat:@"%@/",[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject]],[[NSUserDefaults standardUserDefaults] objectForKey:@"theme"]];
                }
                ...
                self.path=[NSString stringWithFormat:@"%@%@/chat_send_nor.png",[NSString stringWithFormat:@"%@/",[NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject]],[[NSUserDefaults standardUserDefaults] objectForKey:@"theme"]];
                }
...
-(void)configWithCustomModel:(MessageModel *)object leftImage:(UIImage *)leftImage rightImage:(UIImage *)right
{
    ...
    if (object.isOutgoing) {
        ...
        if ([type isEqualToString:MESSAGE_STR]) {
            ...
            rightBubbleImageView.frame=CGRectMake(WIDTH-80-size.width, 10, size.width+40, size.height+20);
            ...
            if ([type isEqualToString:MESSAGE_VOICE]) {
                ...
                //语音
                rightBubbleImageView.frame=CGRectMake(WIDTH-40-70, 0, 50, 40);
                ...
                if ([type isEqualToString:MESSAGE_IMAGESTR]) {
                    ...
                    rightBubbleImageView.frame=CGRectMake(WIDTH-40-w-40, 10, w+40, h+40);
                    ...
                    if ([type isEqualToString:MESSAGE_BIGIMAGESTR]) {
                        ...
                        rightLocationButton.hidden=YES;
                        rightBubbleImageView.frame=CGRectMake(WIDTH-40-110, 0, 100, 100);
        ...
        if ([type isEqualToString:MESSAGE_STR]) {
            ...
            leftBubbleImageView.frame=CGRectMake(40, 10, size.width+40, size.height+20);
            ...
            if ([type isEqualToString:MESSAGE_VOICE]) {
                ...
                leftBigImageView.hidden=YES;
                leftLocationButton.hidden=YES;
                leftBubbleImageView.frame=CGRectMake(40, 0, 50, 40);
            }else{
                if ([type isEqualToString:MESSAGE_IMAGESTR]) {
                    leftTextView.hidden=YES;
                    ...
                    CGFloat w=[self imageToSize:image].width;
                    CGFloat h=[self imageToSize:image].height;
                    leftBubbleImageView.frame=CGRectMake(40, 10, w+40, h+40);
                    ...
                    if ([type isEqualToString:MESSAGE_BIGIMAGESTR]) {
                        ...
                        leftLocationButton.hidden=YES;
                        leftBubbleImageView.frame=CGRectMake(40, 0, 100, 100);
